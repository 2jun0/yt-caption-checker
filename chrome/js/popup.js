...
function setTagFontSize(fontSize) {
  ...
  chrome.storage.local.set(
    { "YT-SUBTITLE-FILTER_tag-font-size": fontSize },
    () => {}
  );
}
...
function combineRegion(enable) {
  ...
  updateLangOptions();
  chrome.storage.local.set(
    { "YT-SUBTITLE-FILTER_combine-region": enable },
    () => {}
  );
}
langPicker.onchange = () => {
...
color1Display.onclick = () => {
  if (color1Picker.el.style.display == "none")
    color1Picker.el.style.display = "block";
  else color1Picker.el.style.display = "none";
};
color2Display.onclick = () => {
  if (color2Picker.el.style.display == "none")
    color2Picker.el.style.display = "block";
...
combineRegionCheckbox.onchange = () => {
  ...
  sendMessage({
    "YT-SUBTITLE-FILTER_combine-region": combineRegionCheckbox.checked,
  });
};
...
(async () => {
  ...
  chrome.storage.local.get(
    ...
    (items) => {
      ...
      setTagFontSize(
        items["YT-SUBTITLE-FILTER_tag-font-size"] || TagFontSizes[3]
      );
      combineRegion(
